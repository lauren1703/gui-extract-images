...
public class SelectionComponent extends JComponent implements MouseListener, MouseMotionListener,
    ...
    private void paintControlPoints(Graphics g, List<Point> points) {
        ...
        for (Point point: points) {
            g.fillOval(point.x - controlPointRadius / 2, point.y - controlPointRadius / 2, controlPointRadius, controlPointRadius);
        }
    }
    ...
    private void paintMoveGuides(Graphics g, List<Point> controlPoints) {
        ...
        assert selectedIndex >= 0 && selectedIndex < controlPoints.size();

        int indexBefore = (selectedIndex == 0) ? controlPoints.size() - 1: selectedIndex - 1;
        int indexAfter = (selectedIndex + 1) % controlPoints.size();
        g.setColor(liveWireColor);
        g.drawLine(controlPoints.get(indexBefore).x, controlPoints.get(indexBefore).y, mouseLocation.x, mouseLocation.y);
        g.drawLine(controlPoints.get(indexAfter).x, controlPoints.get(indexAfter).y, mouseLocation.x, mouseLocation.y);
    }
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        if (e.getButton() == MouseEvent.BUTTON1) {
            if (model.state().isEmpty() || model.state().canAddPoint()) {
            ...
            if (model.state().canFinish() || model.state().isFinished()) {
                model.undoPoint();
            }
        }
    }
    ...
    public void mousePressed(MouseEvent e) {
        if (e.getButton() == MouseEvent.BUTTON1 && model.state().isFinished()) {
            selectedIndex = model.closestPoint(new Point(e.getX(), e.getY()), (int) Math.pow(controlPointRadius, 2));
        }
    }
