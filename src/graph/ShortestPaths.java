...
public class ShortestPaths<VertexType extends Vertex<EdgeType>, EdgeType extends Edge> {
    ...
    public PathfindingSnapshot extendSearch(int maxToSettle) {
        ...
        if (distances[startId] == -1) {
            distances[startId] = 0;
            frontier.addOrUpdate(startId, 0);
        }

        while (!frontier.isEmpty() && settledVertices < maxToSettle) {
            int frontVertexId = frontier.remove();
            int frontVertexDist = distances[frontVertexId];
            settledIds.set(frontVertexId);
            settledVertices++;

            Vertex<EdgeType> frontVertex = graph.getVertex(frontVertexId);
            for (EdgeType vertexEdges: frontVertex.outgoingEdges()) {
                int adjacentVertexId = vertexEdges.endId();
                int currDist = frontVertexDist + weigher.weight(vertexEdges);

                if (distances[adjacentVertexId] == -1 ||
                        currDist < distances[adjacentVertexId]) {
                    distances[adjacentVertexId] = currDist;
                    predecessors[adjacentVertexId] = frontVertexId;
                    frontier.addOrUpdate(adjacentVertexId, currDist);
                }
            }
        }
